"""
Kimi (Moonshot AI) provider for geocode.
...
class KimiProvider(BaseModel):
    ...
    def __init__(self, config: ModelConfig):
        ...
        if not self.api_key:
            raise ValueError("KIMI_API_KEY environment variable or api_key config is required")
    ...
    async def stream(
        ...
        **kwargs
        ...
        """
        Stream response tokens from Kimi API.
        """
        headers = {
            "Content-Type": "application/json",
            "Authorization": f"Bearer {self.api_key}",
